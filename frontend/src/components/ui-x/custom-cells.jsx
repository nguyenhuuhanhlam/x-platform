...
const isRowEditing = (row, table) => {
	...
	return table.options.meta?.editingRowId === row.original?.id
}
...
export const EditableCell = ({ getValue, row, column, table }) => {
	...
	const onChange = (e) => {
		...
		setValue(val); // Cập nhật state hiển thị (để mượt)
		table.options.meta?.updateData(row.index, column.id, val); // Cập nhật data gốc ngay
	}
